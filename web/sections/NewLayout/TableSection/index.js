...
function TableSection(props) {
  ...
  const getTableData = (endpoint) => {
    axios.get(endpoint).then((response) => {
      response.data[currentLanguage.languageTag]
        ? setData([response.data[currentLanguage.languageTag]])
        ...
        {data && (
          <Grid item xs={12}>
            ...
            >
              <table>
                {headers ? (
                  <thead className={headerTransparentLayout && styles.headerTransparent}>
                    <tr>
                      {headers.map((item) => {
                        return <th key={item._key}>{item[currentLanguage?.languageTag]}</th>
                      })}
                    </tr>
                  </thead>
                ) : (
              ...
              {data[0].dateDaily && (
                <Box sx={{my: 2}}>
                  <Typography align="right" sx={{color: '#77757F'}}>{`${
                    currentLanguage.name === 'EN' ? 'Price as at' : 'Prix au'
                  } ${convertDate(data[0].dateDaily)}`}</Typography>
                </Box>
              )}
            </Scrollbars>
          </Grid>
